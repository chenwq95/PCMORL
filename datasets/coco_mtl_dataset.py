...
class CocoMTLDataset(data.Dataset):
    def __init__(
        self, 
        image_ids_path, 
        input_seq, 
        ...
        print('DataLoader loading category infos')
        with open("data/id2BOC.pkl", "rb") as f:
            self.id2BOC = pickle.load(f)
        self.defaultBOC = np.zeros(80, dtype=int)
    ...
    def __getitem__(self, index):
        ...
        #get bag of categories
        BOC = self.id2BOC.get(str(image_id), self.defaultBOC)
        
        #BOC = np.expand_dims(BOC,-1)
        #print(att_feats.shape, gv_feat.shape, BOC.shape)
        
        return indices, input_seq, target_seq, gv_feat, att_feats, BOC