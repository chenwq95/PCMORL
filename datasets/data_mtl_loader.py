...
from datasets.coco_mtl_dataset import CocoMTLDataset
import samplers.distributed
import numpy as np

def sample_collate(batch):
    indices, input_seq, target_seq, gv_feat, att_feats, BOC = zip(*batch)
    ...
    BOC = torch.cat([torch.from_numpy(b).unsqueeze(0) for b in BOC], 0)
    
    return indices, input_seq, target_seq, gv_feat, att_feats, att_mask, BOC
