...
def precook(s, n=4, out=False, need_split=True):
    """
...
class CiderScorer(object):
    ...
    def __init__(self, df_mode="corpus", test=None, refs=None, n=4, sigma=6.0, cider_cache=None):
        ''' singular instance '''
        self.n = n
        self.sigma = sigma
        self.crefs = []
...
#             self.document_frequency = pkl_file['document_frequency']
    ...
    def my_get_self_cider(self, res, need_split=True, clear_cache=False):

        res_str = [' '.join([str(ind) for ind in tempList]) for tempList in res]
        
        ctest = []
        for i, tempres in enumerate(res):
            if (self.cache_vecs.get(res_str[i], None) is not None):
                temp_vec = self.cache_vecs.get(res_str[i])
                #print("cache_vecs", res_str[i])
            else:
                temp_vec = self.counts2vec(precook(tempres, self.n, need_split=need_split))
                self.cache_vecs[res_str[i]] = temp_vec
                
            ctest.append(temp_vec)
            
        #ctest = [self.counts2vec(precook(_, self.n, need_split=need_split)) for _ in res]
        ...
        if (clear_cache):
            self.clear_cache()
        #print("size of cache scores", len(self.cache_scores))
            
        for tid, test in enumerate(ctest):
            vec, norm, length = test
            for rid, ref in enumerate(ctest):
                vec_ref, norm_ref, length_ref = ref
                combine1 = res_str[tid] + ' ' + res_str[rid]
                combine2 = res_str[rid] + ' ' + res_str[tid]
                ...
                if (self.cache_scores.get(combine1, None) is not None):
                    ...
                    if (res_str[tid] == res_str[rid]):
                        tempscore = np.array([1.0 for _ in range(self.n)])
                        #tempscore = sim(vec, vec_ref, norm, norm_ref, length, length_ref, self.n)
                        #print("same", tempscore)
                    else:
                        tempscore = sim(vec, vec_ref, norm, norm_ref, length, length_ref, self.n)
                        #print("sim", tempscore)
