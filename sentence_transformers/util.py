...
def pair_cosine_score(model, cands, refs, verbose=False, batch_size=64): #, mode="max","mean","all"
    ...
    if not isinstance(refs[0], str):
        ...
        for cand, ref_group in zip(ori_cands, ori_refs):
            ...
            count += len(ref_group)
    cands_embeddings = model.encode(cands, batch_size=batch_size)
    refs_embeddings = model.encode(refs, batch_size=batch_size)
    cosine_scores = 1 - (paired_cosine_distances(cands_embeddings, refs_embeddings))
    all_preds = cosine_scores
    if ref_group_boundaries is not None:
        max_preds = []
        mean_preds = []
        for start, end in ref_group_boundaries:
            max_preds.append(all_preds[start:end].max(axis=0))
            mean_preds.append(all_preds[start:end].mean(axis=0))
        all_max_preds = np.stack(max_preds, axis=0)
        all_mean_preds = np.stack(mean_preds, axis=0)
    return all_max_preds, all_mean_preds
