...
class TransformerEncoderWrapper(nn.Module):
    def __init__(self, encoder):
        ...
        self.layer_norm = LayerNorm(cfg.MODEL.BILINEAR.DIM)
    ...
    #deal with: gv_feat, att_feats = self.encoder_layers(gv_feat, att_feats, att_mask)
    def forward(self, gv_feat, att_feats, att_mask, p_att_feats=None, out_all_layers=False):

        
        
        #convert to L,B,D
        output = att_feats.permute(1,0,2).contiguous()
        #print(att_mask[0,:])
        src_key_padding_mask = (att_mask == 0)

        #print(src_key_padding_mask[0])
        #print("att_mask", att_mask[0])
        for mod in self.encoder.layers:
            output = mod(output, src_key_padding_mask=src_key_padding_mask)

        if self.encoder.norm is not None:
            output = self.encoder.norm(output)
        att_feats = output.permute(1,0,2).contiguous()
...
class AttBasicTransformerModel(BasicModel):
    def __init__(self):
        super(AttBasicTransformerModel, self).__init__()
        ...
        self.p_att_feats2 = nn.Linear(self.att_dim, cfg.MODEL.ATT_HIDDEN_SIZE) \
            if cfg.MODEL.ATT_HIDDEN_SIZE > 0 else None

        # bilinear
        if cfg.MODEL.BILINEAR.DIM > 0:
            #use basic projection layer
            #self.p_att_feats = None
            
            d_model = cfg.MODEL.BILINEAR.DIM
            nhead = cfg.MODEL.BILINEAR.HEAD
            dim_feedforward = 2048
            dropout = 0.4#cfg.MODEL.BILINEAR.ENCODE_BIFEAT_EMB_DROPOUT
            num_encoder_layers = cfg.MODEL.BILINEAR.ENCODE_LAYERS
            activation = "relu"
            
            encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, activation)
            encoder_norm = LayerNorm(d_model)
            trans_encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

            self.encoder_layers = TransformerEncoderWrapper(trans_encoder)
        self.kl_weight = 0.0
        
        self.num_heads = cfg.MODEL.BILINEAR.HEAD
        self.head_dim = cfg.MODEL.BILINEAR.DIM // self.num_heads
    ...
    def preprocess(self, **kwargs):
        ...
        p_att_feats2 = self.p_att_feats2(att_feats) if self.p_att_feats2 is not None else None
        p_att_feats = torch.cat([p_att_feats, p_att_feats2], dim=-1)
        batch_size = p_att_feats.size(0)
        p_att_feats = p_att_feats.view(batch_size, -1, self.num_heads, self.head_dim * 2).transpose(1, 2)
