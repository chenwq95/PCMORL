...
class AttBasicModel(BasicModel):
    def __init__(self):
        ...
        if (cfg.MODEL.TASK == 'IMAGE_CAPTIONING'):
            ...
            #position embed
            #self.position_embed = PositionalEncoding(cfg.MODEL.WORD_EMBED_DIM, 30)#max_len=30
            self.pre_encoder_lstm = nn.LSTM(cfg.MODEL.WORD_EMBED_DIM, cfg.MODEL.WORD_EMBED_DIM//2, batch_first=True, bidirectional=True)
    ...
    def preprocess(self, **kwargs):
        if (cfg.MODEL.TASK != 'IMAGE_CAPTIONING'):
            return self.preprocess_for_paraphrase_task(**kwargs)
...
#         log_probs = log_probs.contiguous()[:, 0]
        ...
        if (kwargs.get("OUT_BEAM")):
            outputs = outputs.contiguous().view(-1, cfg.MODEL.SEQ_LEN)
            log_probs = log_probs.contiguous().view(-1, cfg.MODEL.SEQ_LEN)
