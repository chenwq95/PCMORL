...
class VariationalXLAN(AttBasicModel):
    def __init__(self):
        super(VariationalXLAN, self).__init__()
        self.num_layers = 2
        ...
        if (self.var_type == "OnlyC"):
            ...
            self.use_c = False
            self.use_m = True
        elif (self.var_type == "Vanilla"):
            self.use_c = True
            self.use_m = False
        elif (self.var_type == "GMM"):
            self.use_c = False
            self.use_m = True
        else:
...
#             self.m_emdder = nn.Embedding(1, cfg.MODEL.ATT_FEATS_EMBED_DIM)
#             self.m_ctx2att = nn.Linear(self.att_dim, cfg.MODEL.ATT_HIDDEN_SIZE_FORCAT)
#             self.m_attn = SimpleAttention(self.att_dim, cfg.MODEL.ATT_HIDDEN_SIZE_FORCAT)
...
#         self.kl_weight.requires_grad = False
    ...
    def get_posterior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, seq=None):
        z_m, z_c = None, None
        c_mu, c_logvar, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar = None, None, None, None, None, None
        ...
        if (self.use_c):
            ...
            c_h = (c_output * mask).sum(dim=1) / mask.sum(dim=1)
            c_mu = self.target_logit_mu(c_h)
            c_logvar = self.target_logit_logvar(c_h)
            z_c = self.sample_z(c_mu, c_logvar)
        if (self.use_m):
        ...
        return z_m, z_c, z_final, c_mu, c_logvar, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar
    ...
    def get_prior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, batch_size=None):
        ...
        if (self.use_c):
            z_c = self.sample_z_standard(batch_size)
        if (self.use_m):
            proj_gv = self.m_proj(gv_feat)
            m_prior_mu, m_prior_logvar = self.get_m_prior(proj_gv)
    ...
    def get_var_mu_loss(self, m_prior_mu):
        
        temp_m_div = torch.abs(m_prior_mu) - self.lambda_mu
        #version1, elementwise constraint
        var_m_mu_loss = torch.mean(torch.sum(torch.where(temp_m_div > 0, temp_m_div, torch.zeros_like(temp_m_div)), 1))
            
        return var_m_mu_loss
            
        
    def get_var_losses(self, c_mu, c_logvar, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar):
        
        var_m_loss, var_m_mu_loss, var_c_loss = 0, 0, 0
        if (self.kl_weight > 0):
            if (self.use_c):
                var_c_loss = torch.mean(0.5 * torch.sum(torch.exp(c_logvar) + c_mu**2 - 1 - c_logvar, 1))
    ...
    def forward(self, **kwargs): 
        ...
        z_m, z_c, z_final, c_mu, c_logvar, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar = self.get_laten_variables(gv_feat=gv_feat, att_feats=att_feats, att_mask=att_mask, seq=seq, batch_size=batch_size, stype="posterior")
        var_m_loss, var_m_mu_loss, var_c_loss = self.get_var_losses(c_mu, c_logvar, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar)
        ...
        return outputs, var_m_loss, var_m_mu_loss, var_c_loss
    ...
    def Forward(self, **kwargs):
        ...
        #print(h_att.shape, att_feats.shape, att_mask.shape, p_att_feats.shape)
    ...
    def decode_beam(self, **kwargs):
        ...
        if ("z_final" in kwargs.keys()):
            ...
            z_m, z_c, z_final = self.get_laten_variables(gv_feat=gv_feat, att_feats=att_feats, att_mask=att_mask, batch_size=batch_size, stype="prior")
