...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        ...
        #print(x.shape, self.dropout(sublayer(self.norm(x))).shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, add_to_buffer=False):
        for i,layer in enumerate(self.layers):
            #print(i, x.shape, memory.shape, src_mask.shape, tgt_mask.shape, add_to_buffer)
            x = layer(x, memory, src_mask, tgt_mask, add_to_buffer=add_to_buffer)
        return self.norm(x)
...
class DecoderLayer(nn.Module):
    ...
    def init_buffer(self, batch_size):
        self.buffer_keys = torch.zeros((batch_size, 0, self.size)).cuda()
    ...
    def forward(self, x, memory, src_mask, tgt_mask, add_to_buffer=False):
        ...
        #print(x.shape, m.shape, src_mask.shape)
...
#         if self.buffer_keys is not None:
#             kv = torch.cat([self.buffer_keys, x], dim=1)
#         else:
#             kv = x
        
#         print(x.shape, kv.shape, tgt_mask.shape)
#         x = self.sublayer[0](x, lambda x: self.self_attn(x, kv, kv, tgt_mask))
#         #print(x.shape, m.shape, src_mask.shape)
#         x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask))
