...
class GMCVAEXLANDiverse(AttBasicModel):
    def __init__(self):
        super(GMCVAEXLANDiverse, self).__init__()
        self.num_layers = 2
        ...
        self.component_num = cfg.MODEL.VAR.component_num
        
        #posterior encoding x and y for q(k|x,y) and q(z|x,y,k)
        ...
        #for q(k|x,y)
        self.m_posterior_k = nn.Linear(cfg.MODEL.VAR.RNN_SIZE * 3, self.component_num)
        self.component_embed = nn.Embedding(self.component_num, cfg.MODEL.VAR.component_embed_size)
        
        #for q(z|x,y,k)
        self.m_posterior_logit_mu = nn.Linear(cfg.MODEL.VAR.RNN_SIZE * 3 + cfg.MODEL.VAR.component_embed_size, cfg.MODEL.VAR.LATENT_SIZE)
        self.m_posterior_logit_logvar = nn.Linear(cfg.MODEL.VAR.RNN_SIZE * 3 + cfg.MODEL.VAR.component_embed_size, cfg.MODEL.VAR.LATENT_SIZE)
        
        #for p(k|x)
        self.m_prior_k = nn.Linear(cfg.MODEL.VAR.RNN_SIZE, self.component_num)
        
        #for p(z|x,k), approximated to p(z|k)
        self.m_prior_logit_mu = nn.Embedding(self.component_num, cfg.MODEL.VAR.LATENT_SIZE)
        self.m_prior_logit_logvar = nn.Embedding(self.component_num, cfg.MODEL.VAR.LATENT_SIZE)
...
#         self.kl_weight.requires_grad = False
    ...
    def get_posterior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, seq=None):
        ...
        m_posterior_k_probs, m_prior_k_probs = None, None
        ...
        #q(k|x,y)
        mixed_fork = torch.cat([proj_gv, m_h], dim=-1)
        m_posterior_k = self.m_posterior_k(mixed_fork)
        m_posterior_k_probs = torch.softmax(m_posterior_k, dim=-1)
        
        #sample a k
        sampled_posterior_k = torch.multinomial(m_posterior_k_probs, 1).squeeze(-1)
        m_posterior_k_embedding = self.component_embed(sampled_posterior_k)
        
        #q(z|x,y,k)
        mixed_forz = torch.cat([proj_gv, m_h, m_posterior_k_embedding], dim=-1)
        m_posterior_mu = self.m_posterior_logit_mu(mixed_forz)
        m_posterior_logvar = self.m_posterior_logit_logvar(mixed_forz)
        z_m = self.sample_z(m_posterior_mu, m_posterior_logvar)
        
        if (self.kl_weight > 0):
            #p(z|x,k) approximated to p(z|k)
            #mixed_for_prior = torch.cat([proj_gv, m_posterior_k_embedding], dim=-1)
            m_prior_mu, m_prior_logvar = self.get_m_prior(sampled_posterior_k)
            m_prior_k = self.m_prior_k(proj_gv)
            m_prior_k_probs = torch.softmax(m_prior_k, dim=-1)

        z_final = z_m
            
        return z_m, z_final, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar, m_posterior_k_probs, m_prior_k_probs
    

    def get_m_prior(self, sampled_posterior_k):
        m_prior_mu = self.m_prior_logit_mu(sampled_posterior_k)
        m_prior_logvar = self.m_prior_logit_logvar(sampled_posterior_k)
    ...
    def get_prior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, batch_size=None):
        ...
        sampled_prior_k = torch.multinomial(m_prior_k_probs, 1).squeeze(-1)
        
        #p(z|k,x)
        m_prior_mu, m_prior_logvar = self.get_m_prior(sampled_prior_k)
    ...
    def get_var_losses(self, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar, m_posterior_k_probs, m_prior_k_probs):
        ...
        if (self.kl_weight > 0):
            ...
            var_m_k_loss = (m_posterior_k_probs * torch.log(m_posterior_k_probs / (m_prior_k_probs + 1e-10) + 1e-10)).sum(dim=-1).mean()
            #print(var_m_loss, var_m_k_loss)
            #print(m_posterior_k_probs[0], m_prior_k_probs[0])
            
            var_m_loss = self.kl_weight * (var_m_loss + var_m_k_loss)
    ...
    def forward(self, **kwargs): 
        ...
        z_m, z_final, m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar, m_posterior_k_probs, m_prior_k_probs = self.get_laten_variables(gv_feat=gv_feat, att_feats=att_feats, att_mask=att_mask, seq=seq, batch_size=batch_size, stype="posterior")
        var_m_loss = self.get_var_losses(m_posterior_mu, m_posterior_logvar, m_prior_mu, m_prior_logvar, m_posterior_k_probs, m_prior_k_probs)
