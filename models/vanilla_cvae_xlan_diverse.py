...
class VanillaCVAEXLANDiverse(AttBasicModel):
    def __init__(self):
        super(VanillaCVAEXLANDiverse, self).__init__()
        self.num_layers = 2
        ...
        if (cfg.MODEL.VAR.decoder_with_preference):
            ...
            self.preference_embedding_layer = nn.Embedding(2, cfg.MODEL.VAR.preference_embed_size)
            full_latent_size += cfg.MODEL.VAR.preference_embed_size
...
#         self.kl_weight.requires_grad = False
    ...
    def get_posterior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, seq=None):
        ...
        return z_m, z_final, m_posterior_mu, m_posterior_logvar
    ...
    def get_prior_laten_variables(self, gv_feat=None, att_feats=None, att_mask=None, batch_size=None):
        ...
        z_m = self.sample_z_standard(batch_size)
        z_final = z_m
    ...
    def get_var_losses(self, m_posterior_mu, m_posterior_logvar):
        
        var_m_loss = 0
        if (self.kl_weight > 0):

            var_m_loss = torch.mean(0.5 * torch.sum(torch.exp(m_posterior_logvar) + m_posterior_mu**2 - 1 - m_posterior_logvar, 1))
    ...
    def forward(self, **kwargs): 
        ...
        z_m, z_final, m_posterior_mu, m_posterior_logvar = self.get_laten_variables(gv_feat=gv_feat, att_feats=att_feats, att_mask=att_mask, seq=seq, batch_size=batch_size, stype="posterior")
        var_m_loss = self.get_var_losses(m_posterior_mu, m_posterior_logvar)
    ...
    def Forward(self, **kwargs):
        ...
        #print(self.proj_refine_hatt, h_att.shape)
