...
import models
import datasets
import datasets.coco_mtl_dataset
import datasets.data_mtl_loader
...
class Trainer(object):
    ...
    def setup_dataset(self):
        self.coco_set = datasets.coco_mtl_dataset.CocoMTLDataset(            
    ...
    def setup_loader(self, epoch):
        self.training_loader = datasets.data_mtl_loader.load_train(
    ...
    def make_kwargs(self, indices, input_seq, target_seq, gv_feat, att_feats, att_mask, BOC):
    ...
    def display(self, iteration, data_time, batch_time, losses, task1_losses, loss_info):
        if iteration % cfg.SOLVER.DISPLAY != 0:
            return
        if self.distributed and dist.get_rank() > 0:
            return
        info_str = ' (DataTime/BatchTime: {:.3}/{:.3}) losses = {:.5}, task1_losses = {:.5}'.format(data_time.avg, batch_time.avg, losses.avg, task1_losses.avg)
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            logit, task1_loss, task1_logits = self.model(**kwargs)
        ...
        return loss, loss_info, task1_loss
    ...
    def train(self):
        ...
        for epoch in  range(cfg.SOLVER.MAX_EPOCH):
            ...
            for _, (indices, input_seq, target_seq, gv_feat, att_feats, att_mask, BOC) in enumerate(self.training_loader):
                ...
                kwargs = self.make_kwargs(indices, input_seq, target_seq, gv_feat, att_feats, att_mask, BOC)
                loss, loss_info, task1_loss = self.forward(kwargs)
                
                all_loss = loss + task1_loss
                all_loss.backward()
                ...
                task1_losses.update(task1_loss.item())
                self.display(iteration, data_time, batch_time, losses, task1_losses, loss_info)
                iteration += 1
