...
class BeamSearch:
    ...
    def search(self,
        ...
        #print("last_log_probabilities", last_log_probabilities.size())
        ...
        for key, state_tensor in state.items():
            if (key in ["encoder_outputs", "source_mask", "decoder_hidden", "decoder_context"]):
            
                if (state_tensor.dim() == 3):
                    n_layer = state_tensor.size(0)
                    _, _, *last_dims = state_tensor.size()

                    #print(key, state_tensor.size())
                    state[key] = state_tensor.\
                            unsqueeze(2).\
                            expand(n_layer, batch_size, self.beam_size, *last_dims).\
                            reshape(n_layer, batch_size * self.beam_size, *last_dims)
                else:
                ...
                if (key in ["encoder_outputs", "source_mask", "decoder_hidden", "decoder_context"]):
                    if (state_tensor.dim() == 3):
                        ...
                        expanded_backpointer = backpointer.\
                                view(1, batch_size, self.beam_size, *([1] * len(last_dims))).\
                                expand(n_layer, batch_size, self.beam_size, *last_dims)
