...
@Model.register("custom_simple_seq2seq")
class SimpleSeq2Seq(Model):
    ...
    def forward_multi_ref(self, source_tokens: Dict[str, torch.LongTensor],
                target_tokens_list = None) -> Dict[str, torch.Tensor]:
        
        state = self._encode(source_tokens)

        output_dict = {}
        if target_tokens_list and self.training:
            ...
            for j in range(N_target):
                ...
                temp_output_dict = self._forward_loop(state, target_tokens)
                if (output_dict is None):
                    output_dict = temp_output_dict
                else:
                    output_dict["loss"] += temp_output_dict["loss"]
            
            output_dict["loss"] /= len(target_tokens_list)
    ...
    def forward(self,  # type: ignore
        ...
        if (self._multi_ref):
            return self.forward_multi_ref(source_tokens, target_tokens)
        else:
            target_tokens['tokens'] = target_tokens['tokens'].unsqueeze(1)
            return self.forward_multi_ref(source_tokens, target_tokens)
        ...
        if not self.training:
            ...
            if (target_tokens and self._bleu2):
                self._bleu2(best_predictions, target_tokens["tokens"])
            if (target_tokens and self._bleu3):
                self._bleu3(best_predictions, target_tokens["tokens"])
            if (target_tokens and self._bleu3):
                self._bleu3(best_predictions, target_tokens["tokens"])
...
#             attended_input = util.weighted_sum(encoder_outputs, input_weights)
            attended_input = encoder_outputs.mean(dim=1)
            decoder_input = torch.cat((attended_input, embedded_input), -1)
