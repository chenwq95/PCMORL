...
class VAESeq2Seq(Model):
    ...
    def __init__(self,
                 ...
                 max_nagative_loss : float = 2.0) -> None:
        ...
        self._max_nagative_loss = max_nagative_loss
    ...
    def forward(self,  # type: ignore
        ...
        if self._global_step > self.kld_start_inc:
            ...
            #negative syntax
                ...
                diff = self._max_nagative_loss - negative_syntax_output_dict["loss"]
                negative_syntax_loss = torch.where(diff>0, diff, torch.zeros_like(diff))
                output_dict["loss"] += negative_syntax_loss
                self.other_loss["negative_syntax_loss"] = float(negative_syntax_loss.detach().cpu().numpy())
            #negative semantic
                ...
                diff = self._max_nagative_loss - negative_semantic_output_dict["loss"]
                negative_semantic_loss = torch.where(diff>0, diff, torch.zeros_like(diff))
                output_dict["loss"] += negative_semantic_loss
                self.other_loss["negative_semantic_loss"] = float(negative_semantic_loss.detach().cpu().numpy())
