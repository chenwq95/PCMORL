...
class DisentangleSeq2Seq(Model):
    ...
    def __init__(self,
        ...
        self.kld_inc = 1 / (10000 - self.kld_start_inc)
...
#         target_syntax_state["encoder_outputs"] = self.sample_z(target_syntax_mu, target_syntax_logvar)
        target_syntax_state = {"source_mask" : state["source_mask"],
                        "encoder_outputs" : self.sample_z(syntax_mu, syntax_logvar)}
        
#         target_semantic_state = {"source_mask" : target_state["source_mask"],
#                                  "encoder_outputs" : target_state["semantic_encoder_outputs"]}
        
#         target_syntax_state = self._syntax_encode(target_tokens)
#         target_semantic_state = self._semantic_encode(target_tokens)
