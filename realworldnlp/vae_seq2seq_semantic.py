...
@Model.register("vae_seq2seq_semantic")
class VAESeq2Seq(Model):
    ...
    def __init__(self,
                 ...
                 syntax_encoder: Seq2VecEncoder,
                 semantic_encoder: Seq2VecEncoder,
...
#         print("probs.device", probs.device)
    ...
    def encode_return_state(self, tokens):
        ...
        return state, syntax_state, semantic_state, syntax_mu, syntax_logvar
        
    
    def forward_layer_return_output(self, concat_state, tokens, use_entropy = False):     
        ...
        if (self._use_cell):
            ...
            output_dict = self._forward_layer(recon_state, tokens, use_entropy)
    ...
    def forward(self,  # type: ignore
        ...
        state, syntax_state, semantic_state, syntax_mu, syntax_logvar = self.encode_return_state(source_tokens)
        target_state, target_syntax_state, target_semantic_state, target_syntax_mu, target_syntax_logvar = self.encode_return_state(target_tokens)
        ...
        output_dict = self.forward_layer_return_output(concat_state, source_tokens)
        self.other_loss["recon_loss"] = float(output_dict["loss"].detach().cpu().numpy())
        
        #reconstruction target
        concat_target_state = {"source_mask" : source_mask,
                            "encoder_outputs" : torch.cat([target_syntax_state["encoder_outputs"], semantic_state["encoder_outputs"]], dim=-1)}
        target_output_dict = self.forward_layer_return_output(concat_target_state, target_tokens)
        
        output_dict["loss"] += target_output_dict["loss"]
        self.other_loss["target_loss"] = float(target_output_dict["loss"].detach().cpu().numpy())
        ...
        semantic_diff_loss = self.diff_loss(semantic_state["encoder_outputs"], target_semantic_state["encoder_outputs"])
        negative_semantic_diff_loss = self.diff_loss(semantic_state["encoder_outputs"], negative_semantic_outputs)
        
        triplet_SD_diff = self._max_negative_semantic_SD_loss + semantic_diff_loss - negative_semantic_diff_loss
        ...
        negative_syntax_outputs = torch.cat([syntax_mu[1:], syntax_mu[0:1]], dim=0)     
        syntax_diff_loss = self.diff_loss(syntax_mu, target_syntax_mu)
        negative_syntax_diff_loss = self.diff_loss(syntax_mu, negative_syntax_outputs)
        
#         negative_syntax_outputs = torch.cat([syntax_state["encoder_outputs"][1:], syntax_state["encoder_outputs"][0:1]], dim=0) 
#         syntax_diff_loss = F.mse_loss(syntax_mu, negative_syntax_outputs)
#         negative_syntax_diff_loss = F.mse_loss(syntax_mu, negative_syntax_outputs)
        
        triplet_Syntax_diff = torch.abs(syntax_diff_loss-negative_syntax_diff_loss)
        triplet_Syntax_loss = triplet_Syntax_diff
        output_dict["loss"] += triplet_Syntax_loss
        self.other_loss["triplet_Syntax_loss"] = float(triplet_Syntax_loss.detach().cpu().numpy())
        
        #negative semantic
            
        negative_semantic_state = {"source_mask" : source_mask,
                        "encoder_outputs" : torch.cat([syntax_state["encoder_outputs"], negative_semantic_outputs], dim=-1)}
        negative_semantic_output_dict = self.forward_layer_return_output(negative_semantic_state, source_tokens, use_entropy=True)
        target_negative_semantic_state = {"source_mask" : source_mask,
                        "encoder_outputs" : torch.cat([target_syntax_state["encoder_outputs"], negative_semantic_outputs], dim=-1)}
        target_negative_semantic_output_dict = self.forward_layer_return_output(negative_semantic_state, target_tokens, use_entropy=True)
        
        negative_semantic_CE_loss = self._max_negative_semantic_CE_loss * (negative_semantic_output_dict["loss"] + target_negative_semantic_output_dict["loss"])
        ...
        if self._global_step > self.kld_start_inc:
            ...
            if (self._has_semantic and self.training and self._finetune):
            ...
            #negative syntax
                ...
                negative_syntax_state = {"source_mask" : source_mask,
...
#                 triplet_SD_loss = torch.where(triplet_SD_diff>0, triplet_SD_diff, torch.zeros_like(triplet_SD_diff))
        ...
        if not self.training:
            if (self._standard_test):
                syntax_vector = self.sample_z_prior(batch_size)
            else:
                syntax_vector = syntax_state["encoder_outputs"]
                
            #syntax_vector = self.sample_z(syntax_mu, syntax_logvar)
            if (self._has_semantic):
                random_syntax_state = {"source_mask" : source_mask,
            ...
            if (target_tokens and self._rouge):
                self._rouge(best_predictions, target_tokens["tokens"])
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        syntax_final_encoder_output = syntax_encoder_outputs
        
        if (self._has_semantic):
            semantic_encoder_outputs = self._semantic_encoder(encoder_outputs, source_mask)
            semantic_final_encoder_output = semantic_encoder_outputs
        else:
    ...
    def _forward_layer(self,
                      ...
                      use_entropy : bool = False) -> Dict[str, torch.Tensor]:
        ...
        if self.training and (not use_entropy):
        ...
        if target_tokens:
            ...
            loss = self._get_loss(logits, targets, target_mask, use_entropy)
...
#             print(k, v.shape)
    ...
    def _get_loss(logits: torch.LongTensor,
                  ...
                  use_entropy : bool = False) -> torch.Tensor:
        """
        Compute loss.
