...
#         self.to(device)
    ...
    def encode(self, sentences: List[str]=None, batch_tokens=None, batch_size: int = 8, show_progress_bar: bool = None, output_value: str = 'sentence_embedding', convert_to_numpy: bool = False, range_len: bool = False, add_special_tokens=True) -> List[ndarray]:
        """
        ...
        if (range_len):
            ...
            iterator = range(0, len(sentences), len(sentences))
        ...
        for batch_idx in iterator:
            ...
            for text in batch_tokens:
                sentence_features = self.get_sentence_features(text, longest_seq, add_special_tokens=add_special_tokens)
            ...
            for feature_name in features:
                ...
                features[feature_name] = torch.cat(features[feature_name]).cuda()#.to(self.device)
    ...
    def get_sentence_features(self, *features, **keys):
        return self._first_module().get_sentence_features(*features, **keys)
