...
#             semantic_logvar = None
    ...
    def get_recon_diff_loss(self, recontor, target_vector, source_tokens, forward_disc):
        ...
        logits4randloss = self.get_logits4recon_loss(recontor, rand_vector, embeddings)
        
        loss = self._get_loss(logits4randloss, tokens, mask)
        
        if (forward_disc):
            return loss
        else:
            rand4probs = torch.softmax(logits4randloss, dim=-1)
            logits4target = self.get_logits4recon_loss(recontor, target_vectors, embeddings)
            target4probs = torch.softmax(logits4target, dim=-1)
            ...
            diff_loss = -torch.sum(torch.sum(rand4probs * torch.log(target4probs + 1e-9) * extended_mask, dim=-1)) / torch.sum(newmask)
    ...
    def get_encodes_from_vectors(self, source_tokens, source_vector, target_vector_list, disc_model, forward_disc):
        ...
        for j in range(N_target):
            ...
            if (forward_disc):                
                disc_loss += (self.get_u_v_loss(source_vector, target_vector, positive_loss_targets, latent_disc) + self.get_u_v_loss(negative_source_vector, target_vector, negative_loss_targets, latent_disc))
            else:
